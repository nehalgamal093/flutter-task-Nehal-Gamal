...
class CategoriesRemoteDsImpl implements CategoriesRemoteDs {
  @override
  Future<Database> openCategoriesDB() async {
    final documentsDirectory = await getApplicationDocumentsDirectory();
    final path = p.join(
      documentsDirectory.path,
      StringsManager.categoriesSQLStatement,
    );
    return openDatabase(
      path,
      onCreate: (db, version) {
        return db.execute(StringsManager.categoriesSQLStatement);
      },
      version: 1,
    );
  }
  ...
  Future<void> insertCategories() async {
    final db = await openCategoriesDB();
